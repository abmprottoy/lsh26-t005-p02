...
- **Table pagination & search UX** — the stock table paginates at 10 rows/page (Prev/Next); search is debounced (300ms) and shows a spinner over the existing rows while fetching instead of blanking the table.
- **ERP-style UI** — sidebar navigation (Overview / Stock / Returned / Import data / Reports, backend status indicator), a top navbar (breadcrumb, global search, notification bell, profile menu), stat cards with icons, a custom `Dropdown` component replacing all native `<select>`s (matches the app's input styling instead of browser chrome), Inter font, consistent spacing/radius/shadow scale, and a fixed bottom tab bar on mobile instead of a top strip.
