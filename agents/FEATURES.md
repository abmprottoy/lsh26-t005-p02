...
- **ERP-style UI** — sidebar navigation (Overview / Stock / Returned / Import data, backend status indicator), a top navbar (breadcrumb, global search, notification bell, profile menu), stat cards with icons, a custom `Dropdown` component replacing all native `<select>`s (matches the app's input styling instead of browser chrome), Inter font, consistent spacing/radius/shadow scale.
